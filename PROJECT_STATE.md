...
| **04 Sep 2026 - 23:59** | `v1.3.2` (Perbaikan Scrollbar Laptop & Posisi Tombol Keranjang GoFood) | • **Hilangkan Scrollbar Windows/Laptop di GoFood1Page:** Menambahkan class `.hide-scrollbar` dan properti `scrollbarWidth: 'none'` pada seluruh kontainer geser horizontal ('Menu Murah' & 'Pilihan Resto Terpopuler'). Ikon panah dan bar scrollbar browser laptop yang keluar garis putih kini 100% lenyap, tampilan menjadi bersih seperti aplikasi mobile native.<br>• **Perbaikan Tombol Hijau GoFood2Page ke Bagian Bawah:** Menghapus `paddingBottom: '85px'` yang sebelumnya menahan tombol hijau melayang 85px di atas dasar layar; mengubah flex layout container dengan `marginTop: 'auto'` dan `margin: 'auto 16px 16px'`, sehingga tombol '1 item / 22.000' kini menempel rapi tepat di bagian bawah layar seperti di GoFood asli.<br>• **Sinkronisasi GoFood1Page:** Melakukan perbaikan container serupa pada GoFood1Page agar tombol hijau konfirmasi selalu terdorong sempurna ke bawah layar. |
| **05 Sep 2026 - 00:15** | `v1.4.0` (Integrasi Logika Budget Harian GoFood & GoRide dengan GoPay) | • **Single Source of Truth Budget Kategori:** Mengintegrasikan saldo checkout GoFood dan GoRide langsung dengan alokasi harian kategori bersangkutan di GoPay (`state.categories`).<br>  - **GoFood:** Menggunakan sisa kuota harian kategori *Makan & Minum* (`food.todayRemaining` = Rp18.000). Total tagihan Rp25.001 melebihi kuota harian, menampilkan peringatan merah 'Kurang Rp7.001'.<br>  - **GoRide:** Menggunakan sisa kuota harian kategori *Transportasi* (`transport.todayRemaining` = Rp17.000). Tarif Rp58.500 (Cepat) / Rp63.500 (Comfort) melebihi kuota harian, menampilkan peringatan merah 'Kurang Rp41.500' / 'Kurang Rp46.500'.<br>• **Fitur Interaktif Realokasi dari Checkout:** Tombol 'Atur Ulang Budget' langsung membuka `BudgetReallocationSheet` untuk kategori yang kurang (*food* atau *transport*). Penambahan budget (misal Rp30.000 dari kategori Umum) otomatis menambah kuota harian secara instan sehingga status saldo di checkout berubah menjadi hijau (cukup)!<br>• **Pemesanan Over-Budget & Notifikasi GoPay:** Dialog konfirmasi pemesanan over-budget memungkinkan user tetap memesan (`simulateOrderGoFood` / `simulateOrderGoRide`). Saldo GoPay dipotong, pengeluaran tercatat di riwayat transaksi, dan toast notifikasi GoPay yang ramah muncul menginformasikan kondisi anggaran harian terkini. |
| **05 Sep 2026 - 00:42** | `v1.4.1` (Perbaikan Layout Hero Header GoFood & Sizing Mobile/Laptop) | • **Perbaikan Header Runtuh di HP/Mobile (GoFood1Page):** Mengeliminasi `display: flex; flex-direction: column` pada container root yang sebelumnya memicu *flex-shrink* runtuh pada header akibat `overflow: hidden`. Mengembalikan ke layout natural block flow dengan `flexShrink: 0`.<br>• **Perapihan Banner Hero & Teks 'Laper? GoFoodin...':** Memperluas *padding bottom* banner merah menjadi 36px dan memberikan ruang proporsional untuk judul, subteks 'Cek resto-resto enak disekitarmu', serta ilustrasi emoji makanan 3D (`🍛 🍔 🍜 🍹`).<br>• **Presisi Search Bar Overlap:** Search bar 'Lagi mau mamam apa?' dengan `marginTop: -18px` kini menempel sempurna di batas bawah gradien merah tanpa menabrak atau menutupi tombol navigasi atas (`X`, `Kos Daniel`, `Heart`, `Receipt`) maupun teks judul.<br>• **Proteksi Flex Seluruh Halaman Simulasi:** Menambahkan `flexShrink: 0` pada container root dan banner di `GoFood1Page`, `GoFood2Page`, dan `GoRide1Page` agar terisolasi dari kompresi viewport di berbagai resolusi layar smartphone maupun laptop. |
| **05 Sep 2026 - 22:48** | `v1.4.2` (Guided Tour Glow Highlight & Perapihan Warna Report Page) | • **Animasi Pulsing Glow (On-Off Kedap-Kedip):** Implementasi keyframe `demoHighlightPulseCyan`, `demoHighlightPulseGreen`, dan `demoHighlightPulseCoral` (1.5s infinite ease-in-out) di `theme.css`.<br>• **Highlight Alur Interaktif:**<br>  - GoPay Home: Kotak pengeluaran 'Rp1.820...' (glow cyan) & tile ikon Gojek (glow hijau).<br>  - Report Page: Tombol tab 'Pemasukan (Planner)' (glow hijau).<br>  - Gojek Home: Ikon GoRide (glow hijau) & GoFood (glow koral).<br>  - GoRide Flow: Kartu tujuan 'Universitas Indonesia' di GoRide 1 (glow hijau) & tombol 'Konfirmasi titik jemput' di GoRide 2 (glow hijau). GoRide 3 tetap bersih.<br>  - GoFood Flow: Kartu 'Ayam Bakar Madu' (22rb) di GoFood 1 (glow koral) & tombol hijau '1 item 22.000' di GoFood 2 (glow hijau). GoFood 3 tetap bersih.<br>• **Pewarnaan Nominal Pemasukan Menjadi Putih:**<br>  - Alokasi Pemasukan: Nominal bulanan kategori di kanan diubah dari hijau menjadi putih (`#ffffff`).<br>  - Simulasi Kuota Harian: Nominal Makanan, Transport, dan Belanja diubah dari hijau menjadi putih (`#ffffff`).<br>  - Riwayat Pemasukan: Nominal saldo masuk (`+Rp...`) diubah dari hijau menjadi putih (`#ffffff`).<br>• **Penyesuaian Manual User (ReportPage):** Merapikan warna teks tombol header atas menjadi abu-abu (`#7c7c7c`), tag bulan 'Sep 2026' menjadi `#94a3b8`, dan link 'Lihat rekomendasi' menjadi `#94a3b8`.<br>• **Deployment:** Berhasil divalidasi dan di-push ke GitHub untuk auto-deploy Vercel. |
| **06 Sep 2026 - 08:32** | `v1.4.3` (Highlight Glow Ikon Petir FAB HP & Ikon Lainnya Gojek) | • **Glow Ikon Petir FAB Mobile (`MobileFrame.tsx`):** Menambahkan efek pulsing glow cyan (`demo-glow-cyan`) pada tombol Floating Action Button (FAB) mini berikon petir (`⚡`) di mode tampilan smartphone/HP, sehingga pengguna tidak bingung dan langsung menyadari adanya panel evaluator untuk mengontrol navigasi/simulasi alur.<br>• **Glow Ikon 'Lainnya' Gojek Home (`GojekHomePage.tsx`):** Menambahkan efek pulsing glow cyan (`demo-glow-cyan`) pada tombol 'Lainnya' di kartu saldo GoPay pada aplikasi Gojek, memberikan isyarat visual yang jelas kepada penguji cara untuk kembali ke aplikasi GoPay.<br>• **Verifikasi & Build:** Kompilasi TypeScript dan Vite build lulus 100% tanpa warning/error, siap deploy ke produksi. |
| **06 Sep 2026 - 08:39** | `v1.4.4` (Pembaruan Label Panel Simulasi: Pengeluaran & Pemasukan) | • **Pembaruan Teks Panel Simulasi (`MobileFrame.tsx`):** Mengubah label tombol navigasi cepat pada Bottom Sheet Panel Simulasi Evaluator dari teks ringkas `'Keluar'` dan `'Masuk'` menjadi `'Pengeluaran'` dan `'Pemasukan'` secara langsung dan lengkap.<br>• **Optimalisasi Layout Tombol Mobile:** Menyesuaikan ukuran font menjadi `10.5px` dan padding horizontal agar teks `'Pengeluaran'` dan `'Pemasukan'` tetap berada dalam satu baris rapi dan proporsional di berbagai ukuran layar smartphone.<br>• **Verifikasi & Build:** Kompilasi TypeScript dan Vite build lulus 100%. |
